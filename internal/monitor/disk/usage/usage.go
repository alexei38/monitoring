...
// avgDiskUsage получает все элементы из storage,
// по каждому примонтированному устройству, каждой метрики вычисляет среднее число.
func avgDiskUsage(store memory.Storage) *usage.Stats {
	stats := map[string]map[string][]interface{}{}
	for _, item := range store.List() {
		storeStat := item.Value.(*usage.Stats)
...
func AvgStat(ctx context.Context, log *log.Entry, ch chan<- *usage.Stats, interval int, counter int) {
	var iter int
